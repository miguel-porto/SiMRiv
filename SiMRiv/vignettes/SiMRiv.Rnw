...
\title{'SiMRiv'\\(version \Sexpr{version$Version})\\An R package for simulation and analysis of spatially-explicit individual multi-state (animal) movements in any landscape}
...
It thus allows to develop realistic movement models using multi-state markov models, while taking into account environmental complexity.
...
\begin{enumerate}
\item spatial bias (e.g. simulate a biased random walk);
