...
\verb@SiMRiv@ provides a helper function to aid the conversion from vector data (line and polygon shapefiles) to a resistance raster, optionally combining multiple rasters into one.
...
The following example creates a resistance raster with 100m resolution by importing a polygon shapefile,
while assigning different resistance values to each land cover class (given by the \verb@mapvalues@ parameter)
which is in the shapefile field \verb@coverclass@. All pixels not covered by a polygon are assigned a resistance of 0.9.
Note that you can add a margin around the extent of the shapes.
...
version may be too thin for the simulation to adequately proceed. In this example with a river,
resistance is assigned according to the river order field of the shapefile:
...
A better, possibly more realistic option, in the case of a river, may be to use a buffer proportional
to the river order, instead of varying the resistance (but you can do both, obviously). In that case, we're better off loading the shape separately:
...
In the simplest form, conducting simulations is straightforward. Without any other parameters, individuals will be simulated
in an homogeneous environment, start at (0, 0), and have a unit step length for all states.
