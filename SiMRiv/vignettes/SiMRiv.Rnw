...
- multiple interacting individuals' simulations;
- differentiate between physical resistance and habitat suitability;
- turning angle in function of the habitat suitability (i.e., currently, it depends on resistance values. In future versions, we'd like to allow it to be dependent on other features - e.g., habitat type);
- other distributions for turning angles and steplength (currently, we allow ..., ...);
- memory effects.
