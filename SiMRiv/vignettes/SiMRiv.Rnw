...
However, we programmed the software in order to be possible to implement the above-mentioned features in a near future. \bf{We highly encourage interested programmers to join our project (https://github.com/miguel-porto/SiMRiv/)} and help us developing and further extending the package.
