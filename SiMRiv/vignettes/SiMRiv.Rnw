...
\title{'SiMRiv'\\(version \Sexpr{version$Version})
An R package for simulating and modeling spatially-explicit individual multi-state (animal) movements in any landscape}
\author{Miguel Porto, Lorenzo Quaglietta}
\maketitle

\section{INDEX}
1 Conducting basic simulations                                                                  1
2 Multi-state movement simulations                                                              3
3 Simulating in heterogeneous environments                                                      4
3.1 Data import                                                                                 5
3.1.1 Importing shapefiles to use as resistance raster                                          5
... TBC
...
for simulating individual trajectories of species that move without restrictions in the landscape (species moving in a bi-dimensional space, i.e., using the space at 360°) exist, we are aware of no
...
We present the main functions and analyses provided by our package, introduce some case studies, and provide an overview of the possible uses of the package as well as a to do list indicating the main functions we are planning (or would like) to develop.
We assume that you are somehow familiar with the basic concepts of Movement Ecology (for good reviews, see: ...; ...; ...), as well as with the R language and environment. In case you need help for the ’raster’ package, you may want to refer to its vignette (http://cran.r-project.org/web/packages/raster/vignettes/Raster.pdf).
This document consists of 4 main parts. In part I, we introduce how to prepare the data for the simulations. In part II, we report the main functionalities of the software. In part III, we provide some examples of use of the software. In part IV, we provide an overview on main software's potential applications, expected outcomes,
and features we would like to modify or develop by new. This is a work in progress, report of any bug or suggestion is warmly welcomed. We are particularly interested in any collaboration, especially stemming from other programmers who would like to add some functions to our package or extend the already implemented ones.
...
sim.lw.river <- simulate(levy.walker, 20000
...
- parameter estimation through maximum likelihood (e.g., Morales et al 2004; Van de Kerk et al 2015; Patterson et al 2016; ...);
- spatial bias (e.g., to fit a biased random walk);
- multiple individuals' simulations;
- differentiate between physical resistance and habitat suitability;
- steplength NOT in function of the resistance (i.e., currently, steplength depends on resistance values. In future versions, we'd like to allow it to be independent from the resistance, as well as dependent of other features - e.g., habitat type);
- other distributions (e.g., turning angles: von Mises, wrapped normal, wrapped Cauchy, etc.; steplength: log-normal, gamma, Weibull, etc.), or, nonparametric method (Langrock et al. 2015) for estimating the distributions – cf. Van de Kerk et al 2015 and Patterson et al 2016)
- memory effects.
...
- calculate the shortest distance among points along a linear network (e.g., river, road) - useful, for instance, to define linear home range extensions of species moving in linear habitats (e.g., fish or otters);
- calculate core areas (areas of more intense use) of species moving in linear habitats;
- estimate linear home ranges (e.g., linear kernel density estimation);
- step selection function analysis applied to species moving in linear habitats;
- generate random points along linear networks;
