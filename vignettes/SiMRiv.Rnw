...
In the current version, we provide an experimental method for numerically approximating simulation input parameters from real trajectory data,
using an heuristic optimization.
This is done with the function \code{adjustModel}. This function numerically finds combinations of input parameters that minimize
the differences between simulated trajectories and the real trajectory. See the working example in \code{help(adjustModel)}.
...
\begin{enumerate}
\item spatial bias (the software in its current version does not allow to simulate, e.g., a biased random walk);
